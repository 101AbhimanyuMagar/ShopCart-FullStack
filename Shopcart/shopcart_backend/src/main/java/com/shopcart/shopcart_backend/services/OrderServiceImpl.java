...
public void cancelOrderItem(String email, Long orderId, Long orderItemId) {
    ...
    @Override
public List<OrderResponseDTO> getOrdersForAdminPanel(String email) {
    ...
    // ADMIN → only orders related to their products
    List<Order> filteredOrders = new ArrayList<>();

    for (Order order : orders) {

        List<OrderItem> adminItems = order.getOrderItems()
                .stream()
                .filter(item ->
  ...
  @Override 
public byte[] generateInvoicePdf(Long orderId, String email) {
    Order order = orderRepository.findById(orderId)
            .orElseThrow(() -> new ResourceNotFoundException("Order not found"));
